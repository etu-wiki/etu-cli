...
function generateCookbookManifest(rootPath: string, etuYaml: any) {
    ...
    if (!fs.existsSync(rootPath)) {
        ...
        console.log(res)
        fs.cpSync(path.join(__dirname, "viewer", etuYaml.viewer), rootPath, { recursive: true });
    }

    fs.mkdirSync(path.join(rootPath, "p", "3", presentUuid), { recursive: true })
    fs.copyFileSync(path.join(cookbookPath, "manifest.json"), path.join(rootPath, "p", "3", presentUuid, "manifest.json"));
    ...
    server.listen(options.port, async () => {
        ...
        if (process.stdout.isTTY && process.env.NODE_ENV !== "production") {
            ...
            if (parseInt(options.port) !== details.port) {
                message += warning(`This port was picked because ${underline(options.port)} is in use.`);
            }
            console.log(message);
        } else {
        ...
        if (options.modifyViewer) {
            ...
            console.log(info("After editing viewer settings, please reload the page to see the changes.") + "\n");
        }
