...
import { cwd, __dirname, registerShutdown, info, error, warning } from '../utils/common.js';
import { run } from '../utils/node_srv.js';
import { start } from '../utils/ipfs_srv.js';
...
program
    .name('etu run')
    .option('-p, --port <port number>', 'Specify a port on which to listen', "3000")
    .option('--cors', 'Enable CORS, sets \`Access-Control-Allow-Origin\` to \`*\`')
    .option('--ssl-cert <file path>', 'Path to an SSL/TLS certificate\'s public key')
    .option('--ssl-key <file path>', 'Path to the SSL/TLS certificate\'s private key')
...
if (options.cookbook) {
    ...
    if (!fs.existsSync(etuLockYaml)) {
        console.log(error(`No etu-lock.yaml found in ${cwd}  Please run 'etu install' first.`));
        process.exit(1);
    }
    ...
    if (options.ipfs) {
        await start(ETU_PATH, options, etuYaml)
    } else {
        run(ETU_PATH, options, etuYaml)
    }
}